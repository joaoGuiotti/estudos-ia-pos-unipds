...
import { runSeed } from '../config/seed.ts'
...
describe('API Workflow', () => {
  ...
  function createCustomer(customer: Customer, token?: string) {
  ...
  function getCustomers(token?: string) {
    return _testServer.inject({
      method: 'GET',
      url: `${_testServerAddress}/v1/customers`,
      headers: authHeaders(token),
  ...
  function getCustomerById(id: string, token?: string) {
    return _testServer.inject({
      method: 'GET',
  ...
  function updateCustomer(id: string, customer: Customer, token?: string) {
    return _testServer.inject({
      method: 'PUT',
      url: `${_testServerAddress}/v1/customers/${id}`,
      headers: authHeaders(token),
  ...
  async function validateUsersListOrderedByName(usersSent: Customer[]) {
    const res = await getCustomers()
    const statusCode = res.statusCode
    const result = JSON.parse(res.body)

    const sort = (items: Customer[]) =>
      items
  ...
  before(async () => {
    ...
    _adminToken = (JSON.parse(adminRes.body)).token
    _memberToken = (JSON.parse(memberRes.body)).token
  })
  ...
  describe('POST /v1/auth/service-token', () => {
    const ADMIN_SUPER_SECRET = 'change-me-in-production'
    ...
    it('should return 401 for wrong adminSuperSecret', async () => {
      ...
      deepStrictEqual(JSON.parse(res.body).message, 'Invalid admin super secret')
    })
    ...
    it('should return 401 for wrong user credentials', async () => {
      ...
      deepStrictEqual(res.statusCode, 401)
      deepStrictEqual(JSON.parse(res.body).message, 'Invalid credentials')
    })
  })
  ...
  describe('Service token - API access & rate limiting', () => {
    ...
    it(`should allow API access and rate limit after ${MAX_RATE} requests with the service token`, async () => {
      const tokenRes = await _testServer.inject({
        method: 'POST',
        url: `${_testServerAddress}/v1/auth/service-token`,
        payload: { username: 'erickwendel', password: '123123', adminSuperSecret: ADMIN_SUPER_SECRET },
      })
      const { serviceToken } = JSON.parse(tokenRes.body)
      const serviceAuthHeaders = { Authorization: `Bearer ${serviceToken}` }
      const opts = { method: 'GET' as const, url: `${_testServerAddress}/v1/customers`, headers: serviceAuthHeaders }
  ...
  describe('RBAC - member role', () => {
    ...
    it('member cannot create customer (POST /v1/customers)', async () => {
      ...
      deepStrictEqual(JSON.parse(res.body).message, 'Forbidden: Insufficient permissions')
    })
  ...
  describe('GET /v1/health', () => {
    it('should return health without auth', async () => {
      ...
      deepStrictEqual(res.statusCode, 200)
      deepStrictEqual(JSON.parse(res.body), { app: 'customers', version: 'v1.0.1' })
    })
  })
  ...
  describe('GET /v1/customers/:id', () => {
    it('should retrieve a customer by ID', async () => {
      ...
      const { id } = JSON.parse(customerResponse.body)

      const res = await getCustomerById(id)
      deepStrictEqual(res.statusCode, 200)
      deepStrictEqual(JSON.parse(res.body), { id, name: 'Test User', phone: '123456789' })
    })
  ...
  describe('PUT /v1/customers/:id', () => {
    it('should update a customer', async () => {
      ...
      deepStrictEqual(res.statusCode, 200)
      deepStrictEqual(JSON.parse(res.body), { message: `User ${id} updated!`, id })

      const updatedUser = await getCustomerById(id)
      deepStrictEqual(updatedUser.statusCode, 200)
      deepStrictEqual(JSON.parse(updatedUser.body), { id, ...updatedData })
    })
  ...
  describe('DELETE /v1/customers/:id', () => {
    it('should delete a customer', async () => {
      ...
      deepStrictEqual(res.statusCode, 200)
      deepStrictEqual(JSON.parse(res.body), { message: `User ${id} deleted!`, id })
    ...
    it('should return 400 for id invalid', async () => {
      ...
      const res = await deleteCustomer(id)
      deepStrictEqual(res.statusCode, 400)
      deepStrictEqual(JSON.parse(res.body), { message: 'the id is invalid!', id })
    })
  })
})
